...
const LocationSearch: FC<LocationSearchProps> = ({
  ...
  useEffect(() => {
    if(localStorage.hotelSearchValue) {
      setValue(JSON.parse(localStorage.getItem('hotelSearchValue') || ''))
    }
  }, [])
      ...
      return (
        <>
          {state.dataHotel?.map((item, index) => (
            <div className="grid grid-cols-8 mb-1 hover:bg-neutral-100 mb-3" key={index} onClick={()=>{
              handleSelectLocation(item.name || ''); 
              onClickListSearch(item.name || '', item.slug || '');
              localStorage.setItem('hotelSearchValue', JSON.stringify(item.name))
              localStorage.setItem('hotelSlug', JSON.stringify(item.slug))
            }}>
      ...
      >
        ...
        <div className="flex-grow">
          ...
          <span className="block mt-0.5 text-sm text-neutral-400 font-light ">
            ...
            <span className="line-clamp-1">{value.length > 1 ? placeHolder : desc}</span>
          </span>
          {value && showPopover && (
